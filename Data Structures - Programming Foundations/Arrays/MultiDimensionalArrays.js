...
function table_information(number) {
    Promise.all([
        ...
        listItems(countries,number);
        searching_function(countries);
        

    })
}
function listItems(filteredCountries,num=number){
        const [vaccination, cases] = filteredData;
...
function searching_function(countries) {
    let input, filter;
    ...
    input.addEventListener("keyup",()=>{
        ...
        if(filter === ""){
            ...
            listItems(countries.filter((country)=>country.toUpperCase().includes(filter)))
        }
